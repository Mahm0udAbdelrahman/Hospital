...
@section('content')
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <div>
            <h2 class="main-content-title fs-24 mb-1">{{ __('Update Package') }}</h2>
            ...
            <div class="card custom-card">
                ...
                <form class="form-horizontal" method="POST" action="{{ route('packages.update' , $package->id) }}"
                    ...
                    <div class="card-body">
                        ...
                        <div class="input-group mb-3">
                            <input type="file" class="form-control" value="{{ $package->image }}" name="image"
                                id="basic-url" aria-describedby="basic-addon3">
                        </div>
                        @error('image')
                            <div class="alert alert-danger" role="alert">{{ $message }}</div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="status">
                                        ...
                                        <option @if ($package->status == '1') selected @endif value="1">
                                            {{ __('Active') }}
                                        </option>
                                        <option @if ($package->status == '0') selected @endif value="0">
                                            {{ __('Unactive') }}
                                        </option>
                                    </select>
                                    @error('status')
                        ...
                        <div class="card hk-dash-type-1 overflow-hidden">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                @php
                                $translations = $package->getTranslationsArray()[$localeCode];
                                @endphp
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="pane-{{ $localeCode }}"
                                        ...
                                        <div class="form-group">
                                            <label for="address-{{ $localeCode }}">{{ __('Time') }} {{ $localeCode }}</label>
                                            <input type="text" value="{{ $translations['time'] }}"
                                                name="{{ $localeCode }}[time]" id="address-{{ $localeCode }}"
                                                class="form-control">
                                            @error("$localeCode.time")
                                                <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                        </div>

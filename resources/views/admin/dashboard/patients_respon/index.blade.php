                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table id="responsivemodal-DataTable" class="table table-bordered text-nowrap" style="width:100%">
                            <a href="{{ route('patients_respons.create') }}" class="btn btn-primary">Create</a>
                            <thead>
                                <tr>
                                    <th>#</th>
                            ...
                            <tbody>
                                @foreach ($data as $patients_respon)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                    <td>
                                        {{ $patients_respon->name }}

                                    </td>
                                        <td>
                                            {{ $patients_respon->description }}
                                        </td>
                                       
                                        @foreach ($patients_respon->media as $imgs)
                                            <td>
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <a href="{{ route('patients_respons.edit', $patients_respon->id) }}"
                                                    class="btn btn-primary me-2"><i class="las la-edit"></i></a>
                                                <form method="post"
                                                    action="{{ route('patients_respons.destroy', $patients_respon->id) }}">
                                                    @csrf
                                                    @method('delete')

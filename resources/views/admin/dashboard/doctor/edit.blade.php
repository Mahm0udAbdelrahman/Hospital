...
@section('content')
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <div>
            <h2 class="main-content-title fs-24 mb-1">{{ __('Update Doctor') }}</h2>
            ...
            <div class="card custom-card">
                ...
                <form class="form-horizontal" method="POST" action="{{ route('doctors.update' , $doctor->id) }}"
                    ...
                    @endforeach
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select js-example-basic-multiple" name="hospital_id[]"  multiple="multiple">
                                        <option disabled >{{ __('Choose hospital...') }}</option>
                                        @foreach ($hospitals as $hospital)
                                         <option
                                         @foreach($doctorHospital as $hosp)

                                         @selected($hospital->id == $hosp->hospital_id )
                        ...
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select js-example-basic-multiple" name="clinic_id[]"  multiple="multiple">
                                        <option disabled >{{ __('Choose Clinic...') }}</option>
                                        @foreach ($clinics as $clinic)
                                         <option
                                         @foreach($doctorClinic as $hosp)

                                         @selected($clinic->id == $hosp->clinic_id )
                    ...
                    @enderror
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="specialty_id">
                                        ...
                                        @foreach ($specialties as $specialty)
                                         <option @selected($doctor->specialty_id == $specialty->id ) value="{{ $specialty->id }}">
                        ...
                        <div class="input-group mb-3">
                            <input type="text" class="form-control" value="{{ $doctor->year_of_experience }}" name="year_of_experience"
                                id="basic-url" aria-describedby="basic-addon3">
                        </div>
                        ...
                        <div class="input-group mb-3">
                            <input type="file" onchange="showPreview(event)"  class="form-control" value="{{ $doctor->image }}" name="image"
                                id="basic-url" aria-describedby="basic-addon3">
                        </div>
                        @error('image')
                            <div class="alert alert-danger" role="alert">{{ $message }}</div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="status">
                                        ...
                                        <option @if ($doctor->status == '1') selected @endif value="1">
                                            {{ __('Active') }}
                                        </option>
                                        <option @if ($doctor->status == '0') selected @endif value="0">
                                            {{ __('Unactive') }}
                                        </option>
                                    </select>
                                    @error('status')
                        ...
                        <div class="card hk-dash-type-1 overflow-hidden">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                @php
                                $translations = $doctor->getTranslationsArray()[$localeCode] ?? [];
                                @endphp
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="pane-{{ $localeCode }}"
                                        role="tabpanel" aria-labelledby="tab-{{ $localeCode }}">
                                        <div class="form-group">
                                            <label for="doctor_name-{{ $localeCode }}">{{ __('Docotr Name') }} {{ $localeCode }}</label>
                                            <input type="text" value="{{ $translations['doctor_name'] ?? '' }}"
                                                name="{{ $localeCode }}[doctor_name]" id="doctor_name-{{ $localeCode }}"
                                                class="form-control">
                                            @error("$localeCode.doctor_name")
                                                <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                        </div>

                                        <div class="form-group">
                                            <label for="doctor_address-{{ $localeCode }}">{{ __('Docotr Address') }} {{ $localeCode }}</label>
                                            <input type="text" value="{{ $translations['doctor_address'] ?? '' }}"
                                                name="{{ $localeCode }}[doctor_address]" id="doctor_address-{{ $localeCode }}"
                                                class="form-control">
                                            @error("$localeCode.doctor_address")
                                                <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                        </div>

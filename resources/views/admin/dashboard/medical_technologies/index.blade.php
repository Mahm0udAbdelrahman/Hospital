                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table id="responsivemodal-DataTable" class="table table-bordered text-nowrap" style="width:100%">
                            <a href="{{ route('medical_technologies.create') }}" class="btn btn-primary">Create</a>
                            <thead>
                                <tr>
                                    <th>#</th>
                            ...
                            <tbody>
                                @foreach ($medical_technologies as $medical_technology)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                    <td>
                                        {{ $medical_technology->name }}

                                    </td>
                                        <td>
                                            {{ $medical_technology->description }}
                                        </td>
                                       
                                        @foreach ($medical_technology->media as $imgs)
                                            <td>
                                        ...
                                        @endforeach
                                        <td>
                                            @if($medical_technology->status == 1)
                                                {{ __('Active') }}
                                            @else
                                                {{ __('Unactive') }}
                                            @endif
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <a href="{{ route('medical_technologies.edit', $medical_technology->id) }}"
                                                    class="btn btn-primary me-2"><i class="las la-edit"></i></a>
                                                <form method="post"
                                                    action="{{ route('medical_technologies.destroy', $medical_technology->id) }}">
                                                    @csrf
                                                    @method('delete')

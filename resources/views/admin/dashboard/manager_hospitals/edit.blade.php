...
@section('content')
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <div>
            <h2 class="main-content-title fs-24 mb-1">Update Manager Hospital</h2>
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="javascript:void(0)">Dashboard</a></li>
                <li class="breadcrumb-item active" aria-current="page">Manager Hospital</li>
            </ol>
        </div>
    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card custom-card">
                ...
                <form class="form-horizontal" method="POST" action="{{ route('manager_hospitals.update', $manager_hospital->id) }}"
                    enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="card-body">
                        @foreach($manager_hospital->media as $img)
                                ...
                                <div class="col-md-9">
                                    ...
                                    <select class="form-select" name="hospital_id"  >
                                        ...
                                        @foreach ($hospitals as $hospital)
                                            <option  @selected($manager_hospital->hospital_id == $hospital->id ) value="{{ $hospital->id }}">
                        ...
                        <div class="input-group mb-3">
                            <input type="file" onchange="showPreview(event)"  class="form-control" value="{{ $manager_hospital->image }}" name="image"
                            ...
                            <div class="row row-sm">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="position_type">
                                        <option disabled selected>{{ __('Choose Position Type...') }}</option>
                                        <option @if ($manager_hospital->position_type == 'board_of_directors') selected @endif  value="board_of_directors">{{ __('Board Of Directors') }}</option>
                                        <option @if ($manager_hospital->position_type == 'executive_management') selected @endif value="executive_management">{{ __('Executive Management') }}</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="row row-sm">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="status">
                                        ...
                                        <option @if ($manager_hospital->status == '1') selected @endif value="1">
                                            {{ __('Active') }}
                                        </option>
                                        <option @if ($manager_hospital->status == '0') selected @endif value="0">
                                            {{ __('Unactive') }}
                                        </option>
                        ...
                        <div class="card hk-dash-type-1 overflow-hidden">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                @php
                                $translations = $manager_hospital->getTranslationsArray()[$localeCode];
                                @endphp
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="pane-{{ $localeCode }}"
                                        ...
                                        <div class="form-group">
                                            <label for="position-{{ $localeCode }}">{{ __('position') }} {{ $localeCode }}</label>
                                            <input type="text" value="{{ $translations['position'] }}"
                                                name="{{ $localeCode }}[position]" id="position-{{ $localeCode }}"
                                                class="form-control">
                                            @error("$localeCode.position")
                                                <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                        </div>

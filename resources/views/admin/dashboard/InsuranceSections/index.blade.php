                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table id="responsivemodal-DataTable" class="table table-bordered text-nowrap" style="width:100%">
                            <a href="{{ route('InsuranceSections.create') }}" class="btn btn-primary">Create</a>
                            <thead>
                                <tr>
                                    <th>#</th>
                            ...
                            <tbody>
                                @foreach ($data as $InsuranceSection)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                    <td>
                                        {{ $InsuranceSection->name }}

                                    </td>                                                                               
                                        @foreach ($InsuranceSection->media as $imgs)
                                            <td>
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <a href="{{ route('InsuranceSections.edit', $InsuranceSection->id) }}"
                                                    class="btn btn-primary me-2"><i class="las la-edit"></i></a>
                                                <form method="post"
                                                    action="{{ route('InsuranceSections.destroy', $InsuranceSection->id) }}">
                                                    @csrf
                                                    @method('delete')

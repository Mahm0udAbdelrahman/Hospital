...
@section('content')
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <div>
            <h2 class="main-content-title fs-24 mb-1">Update Center</h2>
    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card custom-card">
                ...
                <form class="form-horizontal" method="POST" action="{{ route('centers.update', $center->id) }}"
                    enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="card-body">
                        @foreach($center->media as $img)
                                ...
                                <div class="col-md-9">
                                    ...
                                    <select class="form-select js-example-basic-multiple" name="hospital_id[]"  multiple="multiple">
                                        ...
                                        @foreach ($hospitals as $hospital)
                                            <option 
                                            @foreach($HospitalCenters as $HospitalCenter)
                                            @selected($hospital->id == $HospitalCenter->hospital_id)
                                            @endforeach
                                               value="{{ $hospital->id }}">
                        ...
                        <div class="form-group ">
                            <div class="row row-sm">
                                ...
                                <div class="col-md-9">
                                    ...
                                    <select class="form-select js-example-basic-multiple" name="doctor_id[]"  multiple="multiple">
                                        <option   disabled >{{ __('Choose doctor...') }}</option>
                                        @foreach ($doctors as $doctor)
                                            <option 
                                            @foreach($doctorCenters as $doctorCenter)
                                            @selected($doctor->id == $doctorCenter->doctor_id)
                                            @endforeach
                                               value="{{ $doctor->id }}">
                        ...
                        <div class="input-group mb-3">
                            <input type="file" onchange="showPreview(event)"  class="form-control" value="{{ $center->image }}" name="image"
                            ...
                            <div class="row row-sm">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="status">
                                        ...
                                        <option @if ($center->status == '1') selected @endif value="1">
                                            {{ __('Active') }}
                                        </option>
                                        <option @if ($center->status == '0') selected @endif value="0">
                                            {{ __('Unactive') }}
                                        </option>
                        ...
                        <div class="card hk-dash-type-1 overflow-hidden">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                @php
                                $translations = $center->getTranslationsArray()[$localeCode] ;
                                @endphp

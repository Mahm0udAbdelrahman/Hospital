...
@section('content')
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <div>
            <h2 class="main-content-title fs-24 mb-1">Update Info Health</h2>
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="javascript:void(0)">Dashboard</a></li>
                <li class="breadcrumb-item active" aria-current="page">Info Health</li>
            </ol>
        </div>
    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card custom-card">
                ...
                <form class="form-horizontal" method="POST" action="{{ route('info_healths.update', $info_health->id) }}"
                    enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="card-body">
                        @foreach($info_health->media as $img)
                        <img src="{{ $img->getUrl() }}" id="img-prv" height="150" width="150" >
                        @endforeach

                        <label for="basic-url" class="form-label">image</label>
                        <div class="input-group mb-3">
                            <input type="file" class="form-control" value="{{ $info_health->image }}" name="image"
                            ...
                            <div class="row row-sm">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="status">
                                        ...
                                        <option @if ($info_health->status == '1') selected @endif value="1">
                                            {{ __('Active') }}
                                        </option>
                                        <option @if ($info_health->status == '0') selected @endif value="0">
                                            {{ __('Unactive') }}
                                        </option>
                        ...
                        <div class="card hk-dash-type-1 overflow-hidden">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                @php
                                $translations = $info_health->getTranslationsArray()[$localeCode];
                                @endphp

                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table id="responsivemodal-DataTable" class="table table-bordered text-nowrap" style="width:100%">
                            <a href="{{ route('goal_and_mission.create') }}" class="btn btn-primary">Create</a>
                            <thead>
                                <tr>
                                    <th>#</th>
                            ...
                            <tbody>
                                @foreach ($data as $goal_and_mission)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                    <td>
                                        {{ $goal_and_mission->name }}

                                    </td>
                                        <td>
                                            {{ $goal_and_mission->description }}
                                        </td>
                                       
                                        @foreach ($goal_and_mission->media as $imgs)
                                            <td>
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <a href="{{ route('goal_and_mission.edit', $goal_and_mission->id) }}"
                                                    class="btn btn-primary me-2"><i class="las la-edit"></i></a>
                                                <form method="post"
                                                    action="{{ route('goal_and_mission.destroy', $goal_and_mission->id) }}">
                                                    @csrf
                                                    @method('delete')

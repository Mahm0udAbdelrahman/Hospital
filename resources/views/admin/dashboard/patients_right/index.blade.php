                ...
                <div class="card custom-card">
                    ...
                    <div class="card-body">
                        <table id="responsivemodal-DataTable" class="table table-bordered text-nowrap" style="width:100%">
                            <a href="{{ route('patients_rights.create') }}" class="btn btn-primary">Create</a>
                            <thead>
                                <tr>
                                    <th>#</th>
                            ...
                            <tbody>
                                @foreach ($data as $patients_right)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                    <td>
                                        {{ $patients_right->name }}
                                        ...
                                        <td>
                                            {{ $patients_right->description }}
                                        </td>
                                       
                                        @foreach ($patients_right->media as $imgs)
                                            <td>
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <a href="{{ route('patients_rights.edit', $patients_right->id) }}"
                                                    class="btn btn-primary me-2"><i class="las la-edit"></i></a>
                                                <form method="post"
                                                    action="{{ route('patients_rights.destroy', $patients_right->id) }}">
                                                    @csrf
                                                    @method('delete')

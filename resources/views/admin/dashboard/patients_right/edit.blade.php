...
@section('content')
    ...
    <div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
        <div>
            <h2 class="main-content-title fs-24 mb-1">Update Patients Right</h2>
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="javascript:void(0)">Dashboard</a></li>
                <li class="breadcrumb-item active" aria-current="page">Patients Rights</li>
            </ol>
        </div>
    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card custom-card">
                ...
                <form class="form-horizontal" method="POST" action="{{ route('patients_rights.update', $patients_right->id) }}"
                    enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="card-body">
                        @foreach($patients_right->media as $img)
                        <img src="{{ $img->getUrl() }}" id="img-prv" height="150" width="150" >
                        @endforeach

                        <label for="basic-url" class="form-label">image</label>
                        <div class="input-group mb-3">
                            <input type="file" onchange="showPreview(event)" class="form-control" value="{{ $patients_right->image }}" name="image"
                            ...
                            <div class="row row-sm">
                                ...
                                <div class="col-md-9">
                                    <select class="form-select" name="status">
                                        ...
                                        <option @if ($patients_right->status == '1') selected @endif value="1">
                                            {{ __('Active') }}
                                        </option>
                                        <option @if ($patients_right->status == '0') selected @endif value="0">
                                            {{ __('Unactive') }}
                                        </option>
                        ...
                        <div class="card hk-dash-type-1 overflow-hidden">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                @php
                                $translations = $patients_right->getTranslationsArray()[$localeCode];
                                @endphp

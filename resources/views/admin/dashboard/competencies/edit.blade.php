...
<div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
    <div>
        <h2 class="main-content-title fs-24 mb-1">Update competency</h2>
...
<div class="row row-sm">
    <div class="col-xl-12">
        <div class="card custom-card">
            ...
            <form class="form-horizontal" method="POST" action="{{ route('competencies.update', $competency->id) }}" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="card-body">

                    @foreach($competency->media as $img)
                    <img src="{{ $img->getUrl() }}" id="img-prv" height="150" width="150" >
                    @endforeach

                    <label for="basic-url" class="form-label">image</label>
                    <div class="input-group mb-3">
                        <input type="file" onchange="showPreview(event)"  class="form-control" value="{{ $competency->image }}" name="image" id="basic-url" aria-describedby="basic-addon3">
                    </div>
                    @error('image')
                        ...
                        <div class="row row-sm">
                            ...
                            <div class="col-md-9">
                                <select class="form-select" name="status">
                                    ...
                                    <option @if ($competency->status == '1') selected @endif value="1">
                                        {{ __('Active') }}
                                    </option>
                                    <option @if ($competency->status == '0') selected @endif value="0">
                                        {{ __('Unactive') }}
                                    </option>
                    ...
                    <div class="card hk-dash-type-1 overflow-hidden">
                        ...
                        <div class="tab-content" id="nav-tabContent">
                            @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                            @php
                            $translations = $competency->getTranslationsArray()[$localeCode];
                            @endphp

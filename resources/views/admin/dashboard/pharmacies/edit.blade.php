...
<div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
    <div>
        <h2 class="main-content-title fs-24 mb-1">Update pharmacy</h2>
...
<div class="row row-sm">
    <div class="col-xl-12">
        <div class="card custom-card">
            ...
            <form class="form-horizontal" method="POST" action="{{ route('pharmacies.update', $pharmacy->id) }}" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="card-body">
                    ...
                    <div class="input-group mb-3">
                        <input type="file" onchange="showPrevieww(event)"  class="form-control" value="{{ $pharmacy->logo }}" name="logo" id="basic-url" aria-describedby="basic-addon3">
                    </div>
                    @error('logo')
                    ...
                    <label for="basic-url" class="form-label">image</label>
                    <div class="input-group mb-3">
                        <input type="file" onchange="showPreview(event)" class="form-control" value="{{ $pharmacy->image }}" name="image" id="basic-url" aria-describedby="basic-addon3">
                    </div>
                    @error('image')
                    <div class="alert alert-danger" role="alert">{{ $message }}</div>
                    @enderror
                        ...
                        <div class="row row-sm">
                            ...
                            <div class="col-md-9">
                                <select class="form-select" name="status">
                                    ...
                                    <option @if ($pharmacy->status == '1') selected @endif value="1">
                                        {{ __('Active') }}
                                    </option>
                                    <option @if ($pharmacy->status == '0') selected @endif value="0">
                                        {{ __('Unactive') }}
                                    </option>
                    ...
                    <div class="card hk-dash-type-1 overflow-hidden">
                        ...
                        <div class="tab-content" id="nav-tabContent">
                            @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                            @php
                            $translations = $pharmacy->getTranslationsArray()[$localeCode];
                            @endphp
...
{{-- <script>
    $(document).ready(function() {
        var selectedCountryId = {
            {
                old('country_id', $selectedCountryId)
            }
        };
        var selectedCityId = {
            {
                old('city_id', $selectedCityId)
            }
        };

...
<div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
    <div>
        <h2 class="main-content-title fs-24 mb-1">{{ __('Update Hospital') }}</h2>
...
<div class="row row-sm">
    <div class="col-xl-12">
        <div class="card custom-card">
            ...
            <form class="form-horizontal" method="POST" action="{{ route('hospitals.update' , $hospital->id) }}" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="card-body">
                    @foreach($hospital->media as $img)
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-9">
                                <select class="form-select" name="country_id">
                                    ...
                                    <option @selected($hospital->country_id == $country->id ) value="{{ $country->id }}">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-9">
                                <select class="form-select js-example-basic-multiple" name="center_id[]" multiple="multiple">
                                    ...
                                    <option 
                                    @foreach($centerHoptials as $cen)
                                        
                                    @selected($center->id == $cen->center_id) 
                                    @endforeach
                                    value="{{ $center->id }}">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" value="{{ $hospital->phone }}" name="phone" id="phone" aria-describedby="phoneHelp">
                        @error('phone')
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" class="form-control" value="{{ $hospital->email }}" name="email" id="email" aria-describedby="emailHelp">
                        @error('email')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" value="{{ $hospital->case_treated  }}" name="case_treated" id="case_treated" aria-describedby="case_treatedHelp">
                        @error('case_treated')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" value="{{ $hospital->surgery }}" name="surgery" id="surgery" aria-describedby="surgeryHelp">
                        @error('surgery')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" value="{{ $hospital->medical_staff  }}" name="medical_staff" id="medical_staff" aria-describedby="medical_staffHelp">
                        @error('medical_staff')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        <label for="bed" class="form-label">{{ __('Ded') }}</label>
                        <input type="text" class="form-control" value="{{ $hospital->bed  }}" name="bed" id="bed" aria-describedby="bedHelp">
                        @error('bed')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="file" onchange="showPreview(event)"  class="form-control" value="{{ $hospital->image  }}" name="image" id="image" aria-describedby="imageHelp">
                        @error('image')
                        <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-9">
                                <select class="form-select" name="status">
                                    ...
                                    <option @if ($hospital->status == '1') selected @endif value="1">
                                        {{ __('Active') }}
                                    </option>
                                    <option @if ($hospital->status == '0') selected @endif value="0">
                                        {{ __('Unactive') }}
                                    </option>
                                </select>
                                @error('status')
                    ...
                    <div class="card hk-dash-type-1 overflow-hidden">
                        ...
                        <div class="tab-content" id="nav-tabContent">
                            @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                            @php
                            $translations = $hospital->getTranslationsArray()[$localeCode];
                            @endphp
                            <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="pane-{{ $localeCode }}" role="tabpanel" aria-labelledby="tab-{{ $localeCode }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" value="{{ $translations['address'] }}" name="{{ $localeCode }}[address]" id="address-{{ $localeCode }}" class="form-control">

...
<div class="d-md-flex d-block align-items-center justify-content-between page-header-breadcrumb">
    <div>
        <h2 class="main-content-title fs-24 mb-1">{{ __('Update Insurance') }}</h2>
...
<div class="row row-sm">
    <div class="col-xl-12">
        <div class="card custom-card">
            ...
            <form class="form-horizontal" method="POST" action="{{ route('insurances.update' , $insurance->id) }}" enctype="multipart/form-data">
                @csrf
                @method('put')
                ...
                @endforeach
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-3">
                                <label class="form-label">{{ __('insurance section') }}</label>
                            </div>
                            <div class="col-md-9">
                                <select class="form-select" name="insurance_section_id">
                                    <option disabled selected>{{ __('Choose insurance section...') }}</option>
                                    @foreach ($sections as $section)
                                    <option @selected($insurance->insurance_section_id == $section->id ) value="{{ $section->id }}">
                                        {{ App::getLocale() == 'en' ? $section->getTranslationsArray()['en']['name'] : $section->getTranslationsArray()['ar']['name'] }}
                                    </option>
                                    @endforeach
                                </select>
                                @error('insurance_section_id')
                    ...
                    <div class="input-group mb-3">
                        <input type="file" onchange="showPreview(event)"   class="form-control" value="{{ old('image') }}" name="image"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-9">
                                <select class="form-select js-example-basic-multiple" name="hospital_id[]"  multiple="multiple">
                                    ...
                                    <option
                                    @foreach($insuranceHostipal as $hos)
                                 
                                    @selected($hospital->id == $hos->hospital_id)   
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-9">
                                <select class="form-select" name="status">
                                    ...
                                    <option @if ($insurance->status == '1') selected @endif value="1">
                                        {{ __('Active') }}
                                    </option>
                                    <option @if ($insurance->status == '0') selected @endif value="0">
                                        {{ __('Unactive') }}
                                    </option>
                                </select>
                                @error('status')
                    ...
                    <div class="card hk-dash-type-1 overflow-hidden">
                        ...
                        <div class="tab-content" id="nav-tabContent">
                            @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                            @php
                            $translations = $insurance->getTranslationsArray()[$localeCode];
                            @endphp
